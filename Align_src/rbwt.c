...
unsigned int Rbwt_BWTOccValue(const rbwt_t *bwt, unsigned int index, const unsigned int character) 
{
    ...
    if(index > bwt->textLength){
        fprintf(stderr, "[%s]: occIndex > textLength! (%u > %u)\n", __func__, index, bwt->textLength);
        exit(1);
    }
/*
    if(index == bwt->textLength){
...
*/
    ...
    rbwt2->rbwt1->decodeTable = (DECODE_TABLE_T *)calloc(DNA_OCC_CNT_TABLE_SIZE_IN_WORD, sizeof(DECODE_TABLE_T));
//rbwt2->rbwt0->decodeTable = rbwt2->rbwt1->decodeTable;    
    GenerateDNAOccCountTable(rbwt2->rbwt1->decodeTable);
