...
int index_main(int argc, char *argv[])
{
    ...
    fprintf(stderr, "[snpaln_index]: convert %s file to packedfile!\n",str0); 
    bns_fasta2bntseq(fp_fa, str0);
    gzclose(fp_fa);
    fprintf(stderr, "[snpaln_index]: C part fa2pac %.2f seconds elapse.\n", 
    ...
    fprintf(stderr, "[snpaln_index]: build lookup table from file %s\n", str0);
    LKT_build_lookuptable(str0, lkt);
    fprintf(stderr, "[snpaln_index]: build lookup table finish!\n");
    LKT_output(str1, lkt);
    fprintf(stderr, "[snpaln_index]: output lookup table to %s\n", str1);
    LKT_destroy(lkt);
    ...
    fprintf(stderr, "[snpaln_index]: convert C pac%s to bwt%s\n", str0, str1);
    t = clock(); 
    bwt_bwtgen(str0, str1); 
    fprintf(stderr, "[snpaln_index]: C part pac2bwt %.2f seconds elapse.\n", 
                                      (float)(clock() - t) / CLOCKS_PER_SEC);
                            //update bwt and call sa
    {
        bwt_t *bwt;
        t = clock();
        fprintf(stderr, "[snpaln_index]: C part Update BWT... ");
    ...
    }
    {
        bwt_t *bwt;
        t = clock();
        fprintf(stderr, "[snpaln_index]: C part Construct SA from BWT and Occ... ");
        bwt = bwt_restore_bwt(str1);
    ...
    fprintf(stderr, "[snpaln_index]: Build local pattern %s\n", str0);
    ...
    fprintf(stderr, "[snpaln_index]: convert R pac %s to R bwt %s\n", str1, str2); 
    ...
    strncpy(str1, prefix, 1024);strcat(str1, ".R");
    fprintf(stderr, "[snpaln_index]: build R sa from local pattern %s\n", str0); 
    Rbwt2_save_sa(str1, str0);
    /*************index mixref****************************/
    //str0 = "prefix.ref"
    strncpy(str0, prefix, 1024);strcat(str0, ".ref");
    fprintf(stderr, "[snpaln_index]: build mixRef file %s\n", str0); 
